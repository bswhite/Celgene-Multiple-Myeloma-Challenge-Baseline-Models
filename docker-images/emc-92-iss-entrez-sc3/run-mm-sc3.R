...
cat(paste0("clinical annotation file ", training.validation.file, " has dim: ", dim(anno.tbl)[1], " x ", dim(anno.tbl)[2], "\n"))
print(table(anno.tbl$Study))
write.table(anno.tbl[, c("Patient", "Study")], row.names=FALSE, col.names=TRUE, sep="\t", quote=FALSE)
...
for(col.indx in 1:length(data.set.cols)) {
    for(data.set in na.omit(unique(anno.tbl[,data.set.cols[col.indx]]))) {
        ...
        cat(paste0(data.set, " has dimensions ", dim(tbl)[1], " x ", dim(tbl)[2], "\n"))
        print(colnames(tbl))
