...
run.classifier.eset <- function(test.eset, coefficients, test.anno = NULL, already.log2.transformed = FALSE, train.eset = NULL, train.anno = NULL, threshold = NULL, coefficient.prefactors = NULL, standardize = TRUE) {
  ...
  if(is.null(threshold)) {
    ...
    rownames(train.data) <- featureNames(train.eset)
    colnames(train.data) <- sampleNames(train.eset)
    flag <- unlist(apply(train.data, 1, function(row) any(is.na(row))))
    train.data <- train.data[!flag, ]
    train.data <- t(train.data)
    ## data is now a standardized sample x gene sample

    available <- match(names(coefficients),colnames(train.data))
    ...
    available <- colnames(train.data)[available]
    raw.score.train <- train.data[,available] %*% coefficients[available]
    score.df <- data.frame(ID=rownames(train.data), score=raw.score.train)
  ...
  flag <- unlist(apply(data, 1, function(row) any(is.na(row))))
  if(any(flag)) {
    cat("Some genes were NA\n")
  }
  data <- data[!flag, ]
  data <- t(data)
  cat(paste0("Dim of test data: ", dim(data)[1], " x ", dim(data)[2], "\n"))
  ## data is now a standardized sample x gene sample
