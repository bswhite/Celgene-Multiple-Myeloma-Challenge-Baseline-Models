...
## docker build -t docker.synapse.org/syn7237116/expr-cyto-srf:version1 .
## docker images
## docker login docker.synapse.org
## docker push docker.synapse.org/syn7237116/expr-cyto-srf:version1
...
read.annotation.file <- function() {
    ...
    chal_data_df_anno <- subset(chal_data_df, grepl(name, pattern="globalClinTraining.csv"))
    sapply(chal_data_df_anno$id, function(x) { synGet(x, downloadLocation=download.dir)})
    training.annotation.file <- paste0(download.dir, "globalClinTraining.csv")
...
gr_conditional_rev3 <- function(A,surv) # using A to for consistency with Prof Guan notation; using Prof Guan code as the basis
{
  ...
  cond4i <- surv$t > t_A & e_A == 0 & surv$event==1
  cond5i <- surv$t > t_A & e_A == 0 & surv$event==0
...
guan_rank <- function(survX) # Guan_rank transform  takes in a Surv object and transforms it to the Prof Yuanfang Guan's full rank as used in ALS patient stratification challenge
{
  ...
  return(list(gRank=gRank, r=survDF$r))
##  return(gRank)
}
...
## Look for strong associations between expression and gender for those
## patients with gender annotated.  We will store these to be used to
## impute gender in the validation data.
...
## Translate the gene symbols of the targets we are intersted in to
## Entrez ids (i.e., as used in the expression matrix)
## NSD2 = MMSET/WHSC1
target.gene.symbols <- c("MYC", "TP53", "FGFR3", "WHSC1", "MMSET", "CCND3", "CCND1", "MAF", "MAFB", "NSD2")
## Also, add other gains/losses from Morgan et al. review.
## NIK = MAP3K14
## target.gene.symbols <- unique(c(target.gene.symbols, c("CKS1B", "ANP32E", "LTBR", "NIK", "MAP3K14", "CDKN2C", "FAF1", "FAM46C", "BIRC2", "BIRC3", "RB1", "DIS3", "TRAF3", "CYLD", "WWOX")))
...
clinical.features <- anno.tbl[,c("D_Gender", "D_ISS", "D_Age", "D_PFS", "D_PFS_FLAG")]
rownames(clinical.features) <- anno.tbl$sample.id

## Drop from training any samples that have NA clinical features.
## For the validation phase, we will attempt to do some limited imputation
## (of age and gender).  Here, we will just ignore the limited amount of
## missing data.
num.patients <- nrow(clinical.features)
complete.cases <- na.omit(clinical.features)
missing.data <- unlist(apply(clinical.features, 1, function(row) any(is.na(row))))
num.complete.cases <- nrow(complete.cases)
cat(paste0(num.complete.cases, " of ", num.patients, " have all clinical data specified\n"))
if(any(missing.data)) {
    write.table(clinical.features[missing.data, ], sep="\t", row.names=TRUE, col.names=TRUE)
}

## Combine the expression data with the clinical features to define
## a data matrix to be used for training.

## 18 months
pfs.threshold.days <- (365.25/12)*18
anno.tbl$pfsBasedHighRisk <- unlist(apply(anno.tbl[, c("D_PFS", "D_PFS_FLAG")], 1, function(row) ifelse(any(is.na(row)), NA, ifelse((row[1] < pfs.threshold.days) & (row[2] == 1), TRUE, FALSE))))
clinical.features <- anno.tbl[,c("D_Gender", "D_ISS", "D_Age", "D_PFS", "D_PFS_FLAG", "pfsBasedHighRisk")]
rownames(clinical.features) <- anno.tbl$sample.id
...
training.data <- data[!is.na(data$D_PFS) & !is.na(data$D_PFS_FLAG),]
srv <- Surv(training.data$D_PFS, training.data$D_PFS_FLAG)
gr.obj <- guan_rank(srv)
gR <- gr.obj$gRank
r <- gr.obj$r

lbl <- training.data$pfsBasedHighRisk
...
exclude.cols <- c("pfsBasedHighRisk", "D_PFS", "D_PFS_FLAG")

## trained.gr.rf <- randomForest(x=training.data[, !(colnames(training.data) %in% exclude.cols)], y=gR, keep.forest=TRUE, proximity=FALSE, strata=lbl, sampsize=sampsize, importance=TRUE, replace=FALSE)

## trained.gr.rf <- randomForest(x=training.data[, !(colnames(training.data) %in% exclude.cols)], y=gR, keep.forest=TRUE, proximity=FALSE, strata=lbl, importance=TRUE, replace=FALSE)

trained.gr.rf <- randomForest(x=training.data[, !(colnames(training.data) %in% exclude.cols)], y=gR, keep.forest=TRUE, proximity=FALSE, importance=TRUE, replace=FALSE)

v <- log(r/(1-r))
trained.r.rf <- randomForest(x=training.data[, !(colnames(training.data) %in% exclude.cols)], y=r, keep.forest=TRUE, proximity=FALSE, importance=TRUE, replace=FALSE)

trained.rf <- randomForest(x=training.data[, !(colnames(training.data) %in% exclude.cols)], y=lbl, keep.forest=TRUE, proximity=FALSE, strata=lbl, sampsize=sampsize, importance=TRUE, replace=FALSE)

save.image(".Rdata")

model.state <- list("model" = trained.rf, "target.gene.entrez" = target.gene.entrez, "y.genes" = y.genes)
