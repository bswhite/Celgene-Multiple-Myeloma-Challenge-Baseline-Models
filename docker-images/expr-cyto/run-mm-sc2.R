...
anno.filename <- "sc2_Validation_ClinAnnotations.csv"
anno.filename <- "sc2_LBRound1_ClinAnnotations.csv"
if(use.sequestered.data) {
  chal_data_table <- synTableQuery('select id,name from syn9763945')
  chal_data_df <- chal_data_table@values

  ## Download the annotation file
  chal_data_df_anno <- subset(chal_data_df, grepl(name, pattern=anno.filename))
  sapply(chal_data_df_anno$id, function(x) { synGet(x, downloadLocation=test.dir)})
}

## Read in the annotation file
validation.annotation.file <- paste0(test.dir, anno.filename)
anno.tbl <- read.table(validation.annotation.file, sep=",", header=TRUE)
## print(dim(anno.tbl))
## print(table(anno.tbl$Study))
...
for(col.indx in 1:length(data.set.cols)) {
    for(data.set in na.omit(unique(anno.tbl[,data.set.cols[col.indx]]))) {
        ...
        cat(paste0("nrow of map = ", nrow(map), "\n"))
        write.table(map, sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
...
for(data.set in names(sample.name.mappings)) {
    ...
    cat(paste0("1. ", data.set, " map: ", nrow(map), "\n"))
    map <- na.omit(map)
    cat(paste0("2. ", data.set, " map: ", nrow(map), "\n"))
    ...
    cat(paste0("3. ", data.set, " map: ", nrow(map), "\n"))
    
    map <- map[,c("patient.id", "sample.id")]
    colnames(map) <- c("to", "from")
    map <- map[order(map$from),]
    cat(paste0("4. ", data.set, " map: ", nrow(map), "\n"))
    if(any(duplicated(map$from))) {
      dups <- duplicated(map$from, fromLast = TRUE) | duplicated(map$from, fromLast = FALSE)
      write.table(map[dups, ], row.names=FALSE, col.names=TRUE, quote=FALSE)
    }
    cat(paste0("5. ", data.set, " map: ", nrow(map), "\n"))    
    tbl <- combine_samples_2_patient(tbl, map)
    cat(paste0("6. ", data.set, " map: ", nrow(tbl), "\n"))
    ...
    cat(paste0("7. ", data.set, " map: ", nrow(map), "\n"))
...
## orig.anno.tbl <- anno.tbl
## write.table(file="map.tsv", map, sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE)
anno.tbl <- merge(anno.tbl, map, by.x = "Patient", by.y = "patient.id", all = FALSE)
## write.table(orig.anno.tbl[!(orig.anno.tbl$Patient %in% anno.tbl$Patient), ], sep="\t", col.names=TRUE, row.names=FALSE)
## print(dim(anno.tbl))
...
cat("Calling predict (probabilities)\n")
pred.prob <- as.data.frame(predict(trained.rf, newdata = data, type = "prob"))

cat("Calling predict (class)\n")
pred.cls <- predict(trained.rf, newdata = data)
pred.prob$cls <- as.character(pred.cls)
pred.prob$ID <- rownames(pred.prob)

dir.create(output.dir)

## Output the results with columns ID, raw.score, and high.risk.
output.file <- paste0(output.dir, "/", "predictions.tsv")
predictions.tbl <- pred.prob[,c("ID", "TRUE", "cls")]
