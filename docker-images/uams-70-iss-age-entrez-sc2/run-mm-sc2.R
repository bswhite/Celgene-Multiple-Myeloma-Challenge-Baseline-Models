...
anno.file <- "sc2_INTERNALVALIDATION_clinAnnotation.csv"
...
validation.file <- paste0(test.dir, anno.file)
anno.tbl <- read.table(validation.file, sep=",", header=TRUE)
...
patient.clin.data <- comp[, c("Patient", "D_Age", "D_Gender", "D_ISS")]
colnames(patient.clin.data) <- c("ID", "age", "gender", "iss")

common <- intersect(rownames(patient.clin.data), colnames(expr))
patient.clin.data <- patient.clin.data[common, ]
expr <- expr[, common]

## Invoke UAMS70 to get the continuous score.
## Pass in a dummy threshold to dichotomize into high and low risk,
## but we will ignore.
X <- as.matrix(expr)
y <- NULL
eset <- ExpressionSet(as.matrix(X))
coefficients <- uams.70.get.coefficients()
res <- uams.70.gene(eset, mapping = mapping, coefficients = coefficients, anno = y, threshold = 0)

surv.features <- merge(res$res[, c("ID", "raw.score")], patient.clin.data[, c("ID", "age", "gender", "iss")], by = "ID")
...
tmp <- surv.features
tmp$risk <- risk
tmp$predicted.high.risk <- unlist(lapply(tmp$risk > threshold, function(x) ifelse(x, "TRUE", "FALSE")))
...
predictions.tbl <- tmp[,c("ID", "raw.score", "predicted.high.risk")]
