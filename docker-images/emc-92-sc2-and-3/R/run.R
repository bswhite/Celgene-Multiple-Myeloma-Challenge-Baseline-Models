...
## Execute the EMC-92 model using the trained features.
X <- as.matrix(expr)
y <- NULL
eset <- ExpressionSet(as.matrix(X))
coefficients <- emc.92.get.coefficients()
res <- emc.92.gene(eset, mapping = model.entrezs, coefficients = coefficients, anno = y, threshold = threshold)$res
