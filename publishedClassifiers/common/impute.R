...
impute.missing.clinical.data <- function(anno.tbl) {

    for(col in c("D_ISS", "D_Gender")) {
        cat("Imputing ", col, "\n")
        for(ds in unique(anno.tbl$Study)) {
            cat("Values for ", ds, "\n")
            print(table(anno.tbl[anno.tbl$Study == ds,col]))
        }
        imputed.val <- impute.using.mode(anno.tbl, col, data.set.col = "Study")
        anno.tbl[, col] <- imputed.val
        for(ds in unique(anno.tbl$Study)) {
            cat("Values for ", ds, "\n")
            print(table(anno.tbl[anno.tbl$Study == ds,col]))
        }
    ...
    for(col in c("D_Age")) {
        ...
        for(ds in unique(anno.tbl$Study)) {
            cat("Mean for ", ds, "\n")
            print(mean(anno.tbl[anno.tbl$Study == ds,col]))
            print(table(is.na(anno.tbl[anno.tbl$Study == ds,col])))
        }
        imputed.val <- impute.using.mode(anno.tbl, col, data.set.col = "Study")
        anno.tbl[, col] <- imputed.val
        for(ds in unique(anno.tbl$Study)) {
